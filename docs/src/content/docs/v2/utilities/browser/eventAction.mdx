...
import { TypeSince, Type, Parameters, DescriptionList, Returns, Since } from '../../components'
...
```html
<script type="module">
  import { EventAction } from '@mustib/utils/browser';
...
- If `!` precedes the switch name (negated) it means to negate the switch value
- When using `!` there must be no spaces between it and the switch name
...
- `#modifier` <Since since='2.8.0' /> <br /> 
  a wrapper for [getModifierState()](https://developer.mozilla.org/en-US/docs/Web/API/KeyboardEvent/getModifierState) which runs for `KeyboardEvents` and `PointerEvents` and expects a param of the keys to check if any of them is the modifier.
  - `param` can be a comma separated list of modifiers or an array of modifiers (for [action array syntax](#action-array-syntax)).
