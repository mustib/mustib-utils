...
2. Create an `envVars.ts` file:<br/>
    ...
    ```
3. `EnvVars` takes care of everything by selecting the appropriate value for the current environment, converting it to the correct type, marking it as dynamic if necessary, and handling any errors.<br/>
Then you can use the previous `envVars.ts` file in your code as follows:<br/>
    ```ts
    import { envVars } from './envVars';
    const {port, host} = envVars;
    console.log(port, host); // process.env.NODE_ENV === 'production' ? {port: 5000} : {port: 3000}
    ```
</Steps>
...
export const envVars = new EnvVars({
  ...
  mapObj: {
    ...
    secretValues: {
      parseAs(data) {
        const values = data.varValueForCurrentEnv.split(',');
        return data.currentEnv !== 'test' ? values : values.reverse();
      },
      whenNodeEnvIs: {
...
console.log(envVars) // { port: 5000, host: 'anywhere.com', secretValues: [ '1', '2', '3', '4' ] }
