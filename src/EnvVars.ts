...
function combineEnvVarsSources(sources: EnvVarsSources) {
  ...
  const pushToCombinedEnvVars = (obj: CombinedEnvVarsSources[number]) => combinedEnvVarsSources.push(obj);
...
function getVarFromSources({ whenNodeEnvIs, currentEnv, varName, combinedEnvVarsSources }: {
  ...
  const indexOfVarNameInCombinedEnvVars = combinedEnvVarsSources.findIndex((obj) => {
  ...
  if (indexOfVarNameInCombinedEnvVars === -1) {
    return AppError.throw('Undefined', `(${varName}) in envVars has not assigned value because (${varNameInCombinedEnvVars}) value in the currentEnv which is (${currentEnv}) is undefined and cannot be found in the provided sources`) as never
  }

  const assignedSource = combinedEnvVarsSources[indexOfVarNameInCombinedEnvVars]
...
function parseVarValueFromString({ parseAsStringValue, currentEnv, varNameInAllEnvVarsFromSources, varName,
  assignedSource
}: {
...
function getParseAsHandler({ parseAs, currentEnv, varName, combinedEnvVarsSources,
  whenNodeEnvIs
}: {
  ...
  const { assignedSource, varNameInSources, varValue } = getVarFromSources({ currentEnv, varName, whenNodeEnvIs, combinedEnvVarsSources })

  const typeofParseAs = getTypeof(parseAs);

  switch (typeofParseAs) {
    case "string":
      handler = () => parseVarValueFromString({ currentEnv, parseAsStringValue: parseAs as ParseAsString, varName, varNameInAllEnvVarsFromSources: varNameInSources, assignedSource })
...
export const EnvVars = class _EnvVars {
  constructor({ sources = { fromObject: process.env }, mapObj, enumerable = false, currentEnv = process.env.NODE_ENV! }: ConstructorParams<EnvVarsMapObj>) {
    ...
    Object.entries(mapObj).forEach(([varName, { parseAs = 'string', whenNodeEnvIs, useDynamicValue }]) => {
      const handler = getParseAsHandler({ combinedEnvVarsSources, currentEnv, parseAs, varName, whenNodeEnvIs })
