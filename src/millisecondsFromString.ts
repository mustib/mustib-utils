...
export function millisecondsFromString(string: TimeUnits, options?: Options): number {
  ...
  const milliseconds = parts.reduce((prev, part) => {
    ...
    if (regex === null || regex.groups === undefined) {
      return AppError.throw('Unsupported', `unsupported time part (${part})`) as never
    }

    const unit = regex.groups.unit as keyof typeof timeUnits | undefined
    if (unit === undefined || !Object.hasOwn(timeUnits, unit)) {
      return AppError.throw('Unsupported', `unsupported time unit (${unit}) in (${part})`) as never
    }

    const value = Number(regex.groups.value)
    if (Number.isNaN(value)) {
      return AppError.throw('Unsupported', `unsupported time value (${value}) in (${part})`) as never
    }
